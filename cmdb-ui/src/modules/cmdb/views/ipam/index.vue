...
<script>
import { getIPAMSubnet, triggerIPAMSubnetScan } from '@/modules/cmdb/api/ipam.js'
...
export default {
  ...
  methods: {
    ...
    async handleScanSubnet(node) {
      try {
        await triggerIPAMSubnetScan(node.key)
        this.$message.success(this.$t('cmdb.ipam.scanSubnetSent'))
        setTimeout(() => {
          this.refreshData()
        }, 5000)
        setTimeout(() => {
        ...
        this.$message.error(this.$t('cmdb.ipam.scanSubnetFailed') + ': ' + (e.message || ''))
      }
    },
