...
import { getCIType, getCITypeGroupById, getCITypes } from '@/modules/cmdb/api/CIType'
import { searchCI } from '@/modules/cmdb/api/ci'
import { SUB_NET_CITYPE_NAME } from '@/modules/cmdb/views/ipam/constants'
...
export default {
  ...
  methods: {
    async open(nodeId, type, parentId) {
      ...
      const ciTypeRes = await getCIType(SUB_NET_CITYPE_NAME)
      const ciTypeId = ciTypeRes?.ci_types?.[0]?.id
      if (!ciTypeId) return
      const groupAttr = await getCITypeGroupById(ciTypeId)
      const form = {
        ...this.form
      }
      const formRules = {}
