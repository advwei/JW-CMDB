    ...
    >
      <div class="address-header">
        ...
        <div class="address-header-right">
          <EditAttrsPopover :typeId="addressCITypeId" class="address-header-config" @refresh="refreshColumns">
            <a-button
              type="primary"
              ghost
...
export default {
  ...
  methods: {
    ...
    async getColumns() {
      ...
      try {
        const subscribedRes = await getSubscribeAttributes(this.addressCITypeId)
        if (subscribedRes?.attributes?.length) {
          const subscribedNames = subscribedRes.attributes.map(a => a.name)
          const subscribedAttrMap = {}
          getAttrRes.attributes.forEach(attr => {
            subscribedAttrMap[attr.name] = attr
          })
          subscribedNames.forEach(name => {
            if (subscribedAttrMap[name]) {
              const attr = subscribedAttrMap[name]
              columns.push({
                field: attr.name,
                title: attr.alias || attr.name || '',
                ...attr
