      ...
      <div class="cmdb-ci-main">
        ...
        >
          ...
          <div class="ops-list-batch-action" v-show="!!selectedRowKeys.length">
            ...
            <a-divider type="vertical" />
            <span @click="batchSyncJumpServer">{{ $t('cmdb.ci.syncJumpServer') }}</span>
            <a-divider type="vertical" />
            <span @click="batchAnsibleInit">Ansible自动化</span>
            <a-divider type="vertical" />
            <span @click="batchDelete">{{ $t('delete') }}</span>
            <a-divider type="vertical" />
            <span @click="batchRollback">{{ $t('cmdb.ci.rollback') }}</span>
            <a-divider type="vertical" />
            <span @click="openRecycleForm">{{ $t('cmdb.ci.recycle') }}</span>
        ...
        <RecycleServerForm ref="recycleServerForm" @recycleDone="handleRecycleDone" :ciIds="selectedRowKeys" />
...
import { searchCI, updateCI, deleteCI, syncCIToJumpServer } from '@/modules/cmdb/api/ci'
import { ansibleBatchSetupServer, getAnsiblePlaybooks } from '@/modules/cmdb/api/ansible'
...
import RecycleServerForm from './modules/RecycleServerForm.vue'
  ...
  methods: {
    ...
    batchSyncJumpServer() {
      ...
      this.$confirm({
        ...
        content: that.$t('cmdb.ci.syncJumpServerConfirm'),
        onOk() {
          that.batchSyncJumpServerAsync()
        },
      })
    },
    async batchSyncJumpServerAsync() {
      let successNum = 0
      let errorNum = 0
      let errorMsg = ''
      const key = 'jumpserverSync'

      this.loading = true
      this.loadTip = this.$t('cmdb.ci.syncJumpServerInProgress')

      for (let i = 0; i < this.selectedRowKeys.length; i++) {
        const ciId = this.selectedRowKeys[i]
        await syncCIToJumpServer(ciId, { update_ci: true }, false)
          .then(() => {
            successNum += 1
          })
          .catch((error) => {
            errorNum += 1
            errorMsg = errorMsg + '\n' + `${ciId}:${error.response?.data?.message ?? error.message ?? ''}`
          ...
          .finally(() => {
            this.loadTip = this.$t('cmdb.ci.syncJumpServerInProgress2', {
      ...
      if (!errorNum) {
        this.$message.success(this.$t('cmdb.ci.syncJumpServerSuccess'))
      }
    },
    batchAnsibleInit() {
      const that = this
      this.playbookList = []
      getAnsiblePlaybooks().then((res) => {
        that.playbookList = res.playbooks || []
      }).catch(() => {
        that.playbookList = []
      })
      const modal = this.$confirm({
        ...
        content: h => (
          <div>
            <p>已选择 {this.selectedRowKeys.length} 个资产</p>
            <a-form layout="vertical">
              <a-form-item label="选择剧本">
                <a-select
                  ref="playbookSelect"
                  placeholder="请选择剧本"
                  style="width: 100%"
                ...
                >
                  {that.playbookList.map(pb => (
                    <a-select-option key={pb} value={pb}>{pb}</a-select-option>
                  ))}
                </a-select>
              </a-form-item>
    ...
    openRecycleForm() {
      this.$nextTick(() => {
        this.$refs.recycleServerForm.onOpen()
      })
    },
    handleRecycleDone() {
      this.selectedRowKeys = []
