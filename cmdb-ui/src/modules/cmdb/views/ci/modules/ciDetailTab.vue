...
import { ansibleSetupServer, getAnsiblePlaybooks } from '@/modules/cmdb/api/ansible'
...
export default {
  ...
  methods: {
    ...
    async handleAnsibleInit() {
      const that = this
      let playbookList = []
      try {
        const res = await getAnsiblePlaybooks()
        playbookList = res.playbooks || []
      } catch (e) {
        playbookList = []
      }
      ...
      this.$confirm({
        title: 'Ansible 自动化',
        content: h('div', [
          h('p', `确认对当前资产 [${this.ci.name || this.ci.hostname || this.ciId}] 执行 Ansible 自动化？`),
          h('a-form', { props: { layout: 'vertical' } }, [
            h('a-form-item', { props: { label: '选择剧本' } }, [
              h('a-select', {
                props: { placeholder: '请选择剧本' },
                style: { width: '100%' },
              }, playbookList.map(pb =>
                h('a-select-option', { props: { key: pb, value: pb } }, pb)
              )),
            ]),
            ...
            h('a-form-item', { props: { label: '额外参数（可选）' } }, [
              h('a-input', {
                props: { placeholder: '如 zabbix_server=1.2.3.4 port=10050' },
              }),
            ]),
          ]),
        ]),
