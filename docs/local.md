...
- 启动服务
  ...
  - worker: 
    ...
    - **Windows本地开发修改：**`pipenv run celery -A celery_worker.celery worker -P solo -E -Q one_cmdb_async --loglevel=INFO --logfile=one_cmdb_async.log`
    - 进入**cmdb-api**目录执行 `pipenv run celery -A celery_worker.celery worker -E -Q acl_async --autoscale=2,1 --logfile=one_acl_async.log -D`
    - **Windows本地开发修改**`pipenv run celery -A celery_worker.celery worker -P solo -E -Q acl_async --loglevel=INFO --logfile=one_acl_async.log`
    - **Celery监听多个进程：** 本地开发环境时只要启动一个终端
    - `pipenv run celery -A celery_worker.celery worker -P solo -E -Q one_cmdb_async,acl_async --loglevel=INFO`
