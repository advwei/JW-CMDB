...
class IPAMScanHistoryView(APIView):
    ...
    def post(self):
        try:
            ...
            logging.info("Received scan result: exec_id=%s, ci_id=%s, cidr=%s, ips=%d",
                        data.get('exec_id'), data.get('ci_id'), data.get('cidr'),
                        len(data.get('ips', []) if data.get('ips') else []))
            ScanHistoryManager().add(**data)
            return self.jsonify(code=200)
        except Exception as e:
            logging.error("Failed to process scan result: %s\n%s", e, traceback.format_exc())
            return self.jsonify(code=200)
