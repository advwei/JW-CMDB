...
class SubnetManager(object):
    ...
    @staticmethod
    def trigger_scan(subnet_id):
        from api.lib.cmdb.custom_dashboard import SystemConfigManager

        subnet = CIManager.get_ci_by_id(subnet_id, need_children=False)
        if not subnet:
            abort(404, ErrFormat.ipam_subnet_model_not_found)

        cidr = subnet.get(SubnetBuiltinAttributes.CIDR)
        if not cidr:
            abort(400, "Subnet CIDR not found")

        scan_rule = IPAMSubnetScan.get_by(ci_id=subnet_id, first=True, to_dict=True)
        agent_id = scan_rule.get("agent_id") if scan_rule else None
        if not agent_id:
            abort(400, "No agent assigned to this subnet, please configure scan rule first")

        cfg = SystemConfigManager.get('agent_config')
        agents = {a['agent_id']: a for a in (cfg.get('option', {}).get('agents') or [])} if cfg else {}
        agent = agents.get(agent_id)
        if not agent:
            abort(400, "Agent '{}' not found in config center".format(agent_id))
        ...
        try:
            resp = requests.post(
                "http://{}:{}/scan".format(agent['host'], agent['port']),
                json={"subnet_id": subnet_id, "cidr": cidr, "exec_id": exec_id},
                headers={"X-Auth-Token": agent.get('auth_token', '')},
                timeout=10
            )
            resp.raise_for_status()
        except requests.ConnectionError:
            abort(502, "Cannot connect to agent {}:{}".format(agent['host'], agent['port']))
        except requests.Timeout:
            abort(504, "Agent {}:{} timeout".format(agent['host'], agent['port']))
        except requests.RequestException as e:
            abort(502, "Agent request failed: {}".format(str(e)))
