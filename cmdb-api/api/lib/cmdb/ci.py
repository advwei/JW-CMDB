...
class CIManager(object):
    ...
    @classmethod
    def recycle(cls, ci_ids, delstatus='待回收', _is_admin=False):
        """
        Recycle server: transfer vmserver CI data to delserver model.

        For each selected vmserver CI:
        1. Read its attribute values
        2. Create a new delserver CI with matching attributes + delstatus
        3. Delete JumpServer asset if exists
        4. Release IP address if exists
        5. Delete the original vmserver CI

        :param ci_ids: list of CI IDs to recycle
        :param delstatus: recycle status to set on delserver CI
        :param _is_admin: skip permission checks
        :return: dict with success/failed counts and error details
        """
        current_app.logger.info('Recycle started: ci_ids={}, delstatus={}'.format(ci_ids, delstatus))

        model_config = SystemConfigManager.get('model_config')
        delserver_type_id = (model_config.get('option') or {}).get('delserver_type_id', 54) if model_config else 54
        delserver_type = CITypeCache.get(delserver_type_id)
        if delserver_type is None:
            return abort(400, "delserver CI type (id={}) not found".format(delserver_type_id))

        delserver_attrs = CITypeAttributeManager.get_all_attributes(delserver_type.id)
        delserver_attr_names = {attr.name for _, attr in delserver_attrs}

        delserver_attr_id_map = {attr.name: attr.id for _, attr in delserver_attrs}

        unique_key = AttributeCache.get(delserver_type.unique_id)
        if unique_key is None:
            return abort(400, "delserver unique key not found")
        ...
        for ci_id in ci_ids:
            try:
                ci = CI.get_by_id(ci_id)
                if ci is None:
                    current_app.logger.warning('Recycle CI not found: ci_id={}'.format(ci_id))
                    results['failed'] += 1
                    results['errors'].append({'ci_id': ci_id, 'error': 'CI not found'})
                    continue

                ci_dict_list = cls.get_cis_by_ids([str(ci_id)])
                if not ci_dict_list:
                    current_app.logger.warning('Recycle CI data not found: ci_id={}'.format(ci_id))
                    results['failed'] += 1
                    results['errors'].append({'ci_id': ci_id, 'error': 'CI data not found'})
                    continue

                ci_data = ci_dict_list[0]
                current_app.logger.info('Recycle CI data loaded: ci_id={}, private_ip={}, subnet={}'.format(
                    ci_id, ci_data.get('private_ip'), ci_data.get('subnet')))

                new_ci_dict = {}
                for key, value in ci_data.items():
                    if key in delserver_attr_names and not key.startswith('_'):
                        new_ci_dict[key] = value

                for attr_name in delserver_attr_names:
                    attr_id = delserver_attr_id_map.get(attr_name)
                    attr_obj = AttributeCache.get(attr_name)
                    if attr_obj and attr_obj.is_password and attr_id:
                        decrypted_password = cls.load_password(ci_id, attr_id)
                        if decrypted_password:
                            new_ci_dict[attr_name] = decrypted_password

                new_ci_dict[unique_key.name] = delstatus if unique_key.name == 'delstatus' else new_ci_dict.get(unique_key.name)

                if unique_key.name == 'delstatus':
                    pass
                elif 'delstatus' in delserver_attr_names:
                    new_ci_dict['delstatus'] = delstatus
                ...
                current_app.logger.info('Creating delserver CI: ci_id={}, new_ci_dict_keys={}'.format(
                    ci_id, list(new_ci_dict.keys())))
                ...
                current_app.logger.info('Delserver CI created: ci_id={}, new_ci_id={}'.format(ci_id, new_ci_id))

                try:
                    from api.lib.cmdb.jumpserver import JumpServerAssetSync
                    current_app.logger.info('Deleting JumpServer asset for ci_id={}'.format(ci_id))
                    js_result = JumpServerAssetSync().delete_ci(ci_id)
                    current_app.logger.info('JumpServer asset deleted: ci_id={}, result={}'.format(ci_id, js_result))
                except Exception as e:
                    current_app.logger.warning('Delete JumpServer asset failed for ci_id={}: {}'.format(ci_id, str(e)))
                    results['errors'].append({'ci_id': ci_id, 'type': 'jumpserver_delete', 'error': str(e)})

                try:
                    private_ip = ci_data.get('private_ip')
                    subnet_cidr = ci_data.get('subnet')
                    current_app.logger.info('Releasing IP: ci_id={}, private_ip={}, subnet_cidr={}'.format(
                        ci_id, private_ip, subnet_cidr))
                    if private_ip and subnet_cidr:
                        ...
                        IpAddressManager().assign_ips(
                            [private_ip], None, subnet_cidr,
                            **{IPAddressBuiltinAttributes.ASSIGN_STATUS: IPAddressAssignStatus.UNASSIGNED}
                        )
                        current_app.logger.info('IP released successfully: ci_id={}, private_ip={}'.format(ci_id, private_ip))
                    else:
                        current_app.logger.info('IP release skipped (no private_ip or subnet): ci_id={}'.format(ci_id))
                except Exception as e:
                    current_app.logger.warning('Release IP failed for ci_id={}: {}'.format(ci_id, str(e)))
                    results['errors'].append({'ci_id': ci_id, 'type': 'ip_release', 'error': str(e)})

                cls.delete(ci_id)
                current_app.logger.info('Original vmserver CI deleted: ci_id={}'.format(ci_id))

                results['success'] += 1
                current_app.logger.info('Recycle completed successfully: ci_id={}'.format(ci_id))
            except Exception as e:
                current_app.logger.error('Recycle failed for ci_id={}: {}'.format(ci_id, str(e)))
                results['failed'] += 1
                results['errors'].append({'ci_id': ci_id, 'error': str(e)})

        current_app.logger.info('Recycle finished: success={}, failed={}, errors_count={}'.format(
            results['success'], results['failed'], len(results['errors'])))
        return results

    @classmethod
    def cancel_recycle(cls, ci_ids, _is_admin=False):
        """
        Cancel recycle: transfer delserver CI data back to vmserver model.

        For each selected delserver CI:
        1. Read its attribute values (including decrypted password)
        2. Create a new vmserver CI with matching attributes
        3. Delete the original delserver CI

        :param ci_ids: list of CI IDs to cancel recycle
        :param _is_admin: skip permission checks
        :return: dict with success/failed counts and error details
        """
        current_app.logger.info('Cancel recycle started: ci_ids={}'.format(ci_ids))

        model_config = SystemConfigManager.get('model_config')
        vmserver_type_id = (model_config.get('option') or {}).get('vmserver_type_id', 52) if model_config else 52
        delserver_type_id = (model_config.get('option') or {}).get('delserver_type_id', 54) if model_config else 54

        vmserver_type = CITypeCache.get(vmserver_type_id)
        if vmserver_type is None:
            return abort(400, "vmserver CI type (id={}) not found".format(vmserver_type_id))

        vmserver_attrs = CITypeAttributeManager.get_all_attributes(vmserver_type.id)
        vmserver_attr_names = {attr.name for _, attr in vmserver_attrs}
        vmserver_attr_id_map = {attr.name: attr.id for _, attr in vmserver_attrs}

        unique_key = AttributeCache.get(vmserver_type.unique_id)
        if unique_key is None:
            return abort(400, "vmserver unique key not found")
        ...
        for ci_id in ci_ids:
            try:
                ci = CI.get_by_id(ci_id)
                if ci is None:
                    current_app.logger.warning('Cancel recycle CI not found: ci_id={}'.format(ci_id))
                    results['failed'] += 1
                    results['errors'].append({'ci_id': ci_id, 'error': 'CI not found'})
                    continue
                ...
                ci_dict_list = cls.get_cis_by_ids([str(ci_id)])
                if not ci_dict_list:
                    current_app.logger.warning('Cancel recycle CI data not found: ci_id={}'.format(ci_id))
                    results['failed'] += 1
                    results['errors'].append({'ci_id': ci_id, 'error': 'CI data not found'})
                    continue

                ci_data = ci_dict_list[0]
                current_app.logger.info('Cancel recycle CI data loaded: ci_id={}, private_ip={}'.format(
                    ci_id, ci_data.get('private_ip')))

                new_ci_dict = {}
                for key, value in ci_data.items():
                    if key in vmserver_attr_names and not key.startswith('_'):
                        new_ci_dict[key] = value

                for attr_name in vmserver_attr_names:
                    attr_id = vmserver_attr_id_map.get(attr_name)
                    attr_obj = AttributeCache.get(attr_name)
                    if attr_obj and attr_obj.is_password and attr_id:
                        decrypted_password = cls.load_password(ci_id, attr_id)
                        if decrypted_password:
                ...
                current_app.logger.info('Creating vmserver CI: ci_id={}, new_ci_dict_keys={}'.format(
                    ci_id, list(new_ci_dict.keys())))
                ...
                current_app.logger.info('Vmserver CI created: ci_id={}, new_ci_id={}'.format(ci_id, new_ci_id))

                cls.delete(ci_id)
                current_app.logger.info('Original delserver CI deleted: ci_id={}'.format(ci_id))

                results['success'] += 1
                current_app.logger.info('Cancel recycle completed successfully: ci_id={}'.format(ci_id))
            except Exception as e:
                current_app.logger.error('Cancel recycle failed for ci_id={}: {}'.format(ci_id, str(e)))
                results['failed'] += 1
                results['errors'].append({'ci_id': ci_id, 'error': str(e)})

        current_app.logger.info('Cancel recycle finished: success={}, failed={}, errors_count={}'.format(
            results['success'], results['failed'], len(results['errors'])))
        return results
